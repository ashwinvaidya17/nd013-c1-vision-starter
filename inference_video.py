...
def main(labelmap_path, model_path, tf_record_path, config_path, output_path):
    ...
    - tf_record_path [str]: path to test folder with tf record file to visualize
    ...
    eval_input_config.tf_record_input_reader.input_path[:] = glob.glob(os.path.join(tf_record_path, "*.tfrecord"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--labelmap_path", required=True, type=str, help="path to the label map")
    parser.add_argument("--model_path", required=True, type=str, help="path to the saved model folder")
    parser.add_argument("--tf_record_path", required=True, type=str, help="path to the tf record file")
    parser.add_argument(
        "--config_path", required=False, type=str, default="pipeline.config", help="path to the config file"
    )
    parser.add_argument(
        "--output_path", required=False, type=str, default="animation.mp4", help="path of the saved file"
    )
    args = parser.parse_args()
    main(args.labelmap_path, args.model_path, args.tf_record_path, args.config_path, args.output_path)
